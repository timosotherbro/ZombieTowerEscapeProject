	...
	for (j=((mapy-mapvclip)-mbgy);j<((mapy+maph));j+=mapblockgapy)
	{
		for (i=((mapx-maphclip)-mbgx);i<((mapx+mapw));i+=mapblockgapx) 
		{
			...
			else 
			{
				...
				else if (blkdatapt->bgoff != NULL) {
					printf("Warning: Null bgoff at screen position (%d,%d) in block %d\n", i, j, *mymappt);
				}
			}
			mymappt++;
		}
		if (mapblockstaggerx || mapblockstaggery) 
		{
			...
			for (i=(((mapx-maphclip)-mbgx)+mapblockstaggerx);i<((mapx+mapw));i+=mapblockgapx)
			{
				...
				if (blkdatapt->bgoff != NULL && abmTiles[0] != (ALLEGRO_BITMAP*)blkdatapt->bgoff)
					al_draw_bitmap((ALLEGRO_BITMAP*)blkdatapt->bgoff, i, j, 0);
				else if (blkdatapt->bgoff == NULL)
					printf("Warning: NULL bgoff in staggered draw at (%d,%d), block %d\n", i, j, *mymappt);
...
BLKSTR * myblkpt;
	...
	{
		int bgIndex, fgIndex, fg2Index, fg3Index;

		bgIndex = (int)MapGetlong(mdatpt + i);
		fgIndex = (int)MapGetlong(mdatpt + i + 4);
		fg2Index = (int)MapGetlong(mdatpt + i + 8);
		fg3Index = (int)MapGetlong(mdatpt + i + 12);

		if (maptype == 0)
		{
			int denom = mapblockwidth * mapblockheight * ((mapdepth + 1) / 8);
			bgIndex /= denom;
			fgIndex /= denom;
			fg2Index /= denom;
			fg3Index /= denom;
		...
		// Store indexes as *integer placeholders*, to be patched in MapRelocate2
		myblkpt->bgoff = (ALLEGRO_BITMAP*)(intptr_t)bgIndex;
		myblkpt->fgoff = (ALLEGRO_BITMAP*)(intptr_t)fgIndex;
		myblkpt->fgoff2 = (ALLEGRO_BITMAP*)(intptr_t)fg2Index;
		myblkpt->fgoff3 = (ALLEGRO_BITMAP*)(intptr_t)fg3Index;
...
static int MapRelocate2 (int convertMagicPink, const short int numblockgfx,const short int numblockstr)
{
	...
	while (i)
	{
		...
		index = (intptr_t)myblkstrpt->bgoff;
		if (index != 0)
			myblkstrpt->bgoff = abmTiles[index];

		index = (intptr_t)myblkstrpt->fgoff;
		if (index != 0)
			myblkstrpt->fgoff = abmTiles[index];

		index = (intptr_t)myblkstrpt->fgoff2;
		if (index != 0)
			myblkstrpt->fgoff2 = abmTiles[index];

		index = (intptr_t)myblkstrpt->fgoff3;
		if (index != 0)
			myblkstrpt->fgoff3 = abmTiles[index];
